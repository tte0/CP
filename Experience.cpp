...
mt19937 mt(clock());
///////////////////////////////////////////////////////////////////
int n,m,k,t,q,a,b,x,y,w,ans,dp[N][3];
vi v,p,adj[N];

inline int f(int node=0,int x=0){
    if(dp[node][x]!=-1)return dp[node][x];
    int t=0,sum=0;
    for(const int& i:adj[node])sum+=f(i,0);
    if(x){
        t=sum+(x==1?-1:1)*v[node];
        for(const int& i:adj[node]){
            maxs(t,sum-f(i,0)+f(i,x));
        }
        return dp[node][x]=t;
    }
    else{
        t=sum;
        for(const int& i:adj[node]){
            maxs(t,sum-f(i,0)+v[node]+f(i,1));
            maxs(t,sum-f(i,0)-v[node]+f(i,2));
        }
        return dp[node][x]=t;
    }
}   
