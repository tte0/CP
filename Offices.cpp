...
mt19937 mt(clock());
///////////////////////////////////////////////////////////////////
int n,m,k,t,q,a,b,x,y,w,ans,dp[N][N],dp2[N][N];
string s[N];
...
inline int f(int x,int y){
    ...
    dp[x][y]=0;
    if((s[x][y]-'0')&1)dp[x][y]+=f(x-1,y);
    if((s[x][y]-'0')&2)dp[x][y]+=f(x,y-1);
    dp[x][y]%=MOD;
    return dp[x][y]+1;
}
...
inline int f2(int x,int y){
    if(x<0 || y<0 || x>=n || y>=m)return 0;
    if(dp2[x][y]!=-1)return dp2[x][y]+1;
    dp2[x][y]=0;
    if((s[x][y]-'0')&1)dp2[x][y]+=f2(x-1,y);
    if(y<m-1 && (s[x][y+1]-'0')&2)dp2[x][y]+=f2(x,y+1);
    dp2[x][y]%=MOD;
    return dp2[x][y]+1;
}
