...
const int A=1e9+5;
const int MOD=1e9+7;
const i32 INF=INT32_MAX>>1;
const ll  INFL=INT64_MAX>>1;
const int BLOCK=320;
...
inline vvi operator*(const vvi& a,const vvi& b){
    int na=a.size(),nb=b.size();
    ASSERT(na<1 || nb<1,"invalid n size");
    int ma=a[0].size(),mb=a[0].size();
    ASSERT(ma<1 || mb<1,"invalid m size");
    ASSERT(ma!=nb, "invalid");

    vvi c(na,vi(mb,INFL));
    for(int i=0;i<na;i++){
        for(int j=0;j<mb;j++){
            for(int k=0;k<nb;k++)mins(c[i][j],a[i][k]+b[k][j]);
        }
    }
    return c;
}

inline vvi operator*=(vvi& a,const vvi& b){return a=a*b;}

inline vvi fp(vvi mat,int p){
    vvi ans(n,vi(n,INFL));
...
inline void solve(void){
    ...
    adj=fp(adj,k);
    //debug(adj);
    print((adj[0][n-1]==4611686018427387903)?-1:adj[0][n-1]);
}
