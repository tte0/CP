...
inline void build(int l=0,int r=n-1,int node=1){
    st[node]=vi(v.begin()+l,v.begin()+r+1);
    sort(all(st[node]));
    if(l<r)build(l,mid,node*2),build(mid+1,r,node*2+1);
}

inline int query(int l=0,int r=n-1,int node=1){
    ASSERT(r<l,"r<l");
    if(r<x || y<l)return 0;
    if(x<=l && r<=y)return st[node].end()-upper_bound(all(st[node]),k);
    return query(l,mid,node*2)+query(1+mid,r,1+node*2);
}
