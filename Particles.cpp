...
mt19937 mt(clock());
///////////////////////////////////////////////////////////////////
int n,m,k,t,q,L,x,y,w,ans;
vii a,b;

inline double calc(const ii& i){
    return double(L+a[i.ff].ss+b[i.ss].ss)/(a[i.ff].ff+b[i.ss].ff);
}

inline bool cmp(const ii& i,const ii& j){
    return calc({i.ff,i.ss})<calc({j.ff,j.ss});
}
...
inline void solve(void){
    ...
    vii v;
    for(int i=0;i<n;i++)for(int j=0;j<n;j++)v.pb({i,j});
    sort(all(v),cmp);
    debug(v);
    ...
    for(int i=0;i<v.size() && cnt<k;i++){
        if(vis[v[i].ff] || vis2[v[i].ss])continue;
        vis[v[i].ff]++,vis2[v[i].ss]++;
        cnt++;
        print(v[i].ff+1,v[i].ss+1);
    }
}
