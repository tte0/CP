...
/////////////////////////////////////////////////////////////
int n,m,k,t,q,x,y,arr[N],depth[N],value[N],id[N],place[2*N],sparse[2*N][19],fa[N];
vi adj[N],travel;
...
inline void postProcess(int node,int p){
    //cerr<<"dfs: "<<node<<endl;
    for(auto i:adj[node]){
        //cerr<<"i: "<<i<<endl;
        if(i==p)continue;
    ...
    //cerr<<node<<": ";for(int i=0;i<=n;i++)cerr<<arr[i]<<" ";cerr<<endl; 
    return;
}
...
int32_t main(void){
    ...
    //cerr<<"pre: ";for(int i=0;i<=n;i++)cerr<<arr[i]<<" ";cerr<<endl;    
    postProcess(1,0);
    //cerr<<"post: ";for(int i=0;i<=n;i++)cerr<<arr[i]<<" ";cerr<<endl;
