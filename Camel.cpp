...
constexpr int dx[8]={-3,-2,0,2,3,2 ,0 ,2};
constexpr int dy[8]={0 ,2 ,3,2,0,-2,-3,-2};
mt19937 mt(clock());
///////////////////////////////////////////////////////////////////
int n,m,k,t,q,a,b,x,y,w,ans,cnt[N*N];
vi adj[N*N],bt;
int vis[N*N];

inline bool dfs(int node=0){
    if(node==0 && bt.size()>n*n)return true;
    if(vis[node]++)return false;
    bt.pb(node);
    for(const int& i:adj[node])if(dfs(i))return true;
    bt.pop_back();vis[node]=0;
    return false;
}
...
inline void solve(void){
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            for(int _=0;_<8;i++){
                if(0<=i+dx[_] && i+dx[_]<n && 0<=j+dy[_] && j+dy[_]<n){
                    adj[i*n+j].pb((i+dx[_])*n+j+dy[_]);
                }
            }
        }
    }
