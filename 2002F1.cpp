...
bitset<int(2e7+5)> sieve;
vi v,primes={2};

inline void _init(){
    sieve[0]=sieve[1]=1;
    for(int i=4;i<sieve.size();i++)sieve[i]=1;
    for(int i=3;i<sieve.size();i++){
        if(sieve[i])continue;
        primes.pb(i);
        for(int j=i*i;j<sieve.size();j+=i<<1)sieve[i]=1;
    }
}

inline void solve(void){
    input(n,m,a,b);
    int p=*(--upper_bound(all(primes),n));
    int ans=p*max(a,b)+(p-1)*min(a,b);

    for(int i=1;i<((n-1)|1);i++){
        if(gcd(i,(n-1)|1)==1)maxs(ans,int(max(a,b)*((n-1)|1)+min(a,b)*i));
    }
    print(ans);
}
