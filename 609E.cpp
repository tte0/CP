...
mt19937 mt(clock());
///////////////////////////////////////////////////////////////////
int n,m,k,t,q,a,b,x,y,w,ans,parent[N],bl[N][19],depth[N],vis[N],val[N][19];
vii adj[N],madj[N];
...
inline void dfs(int node=0,int p=0,int d=0,int v=INF){
    if(vis[node]++)return;
    depth[node]=d,val[node][0]=v,bl[node][0]=p;
    for(auto i:adj[node])dfs(i.ff,node,d+1,i.ss);
}
...
inline void binarylifting(){
    for(int j=1;j<19;j++)
        for(int i=0;i<n;i++)
            ...
            val[i][j]=max(val[i][j-1],val[bl[i][j-1]][j-1]);
}
