...
mt19937 mt(clock());
///////////////////////////////////////////////////////////////////
int n,m,k,t,q,a,b,x,y,w,ans,bl[N][20],depth[N];
vi v,p,adj[N];

inline void dfs(int node=0,int d=0){
    depth[node]=d;
    for(int i:adj[node])dfs(i,d+1);
}

inline void binarylifting(){
...
inline void solve(void){
    ...
    v.clear(),p.clear();p.pb(0);
    for(int i=0;i<n-1;i++){cin>>x;p.pb(x-1);}
    for(int i=0;i<n;i++){cin>>x;v.pb(x);}
    for(int i=1;i<n;i++)adj[p[i]].pb(i);

    //debug("p: ",p);
    binarylifting();
    //for(int j=0;j<4;j++){cerr<<"bl: ";for(int i=0;i<n;i++)cerr<<bl[i][j]<<" ";cerr<<endl;}
    ...
    for(int i=0;i<n;i++){
        ...
        if(lca(v[i-1],v[i])==p[v[i]])b[i]=1,cnt++;
    }
    ...
    while(q--){
        ...
        swap(p[x],p[y]);
        if(x>0){
            if(lca(v[x-1],v[x])!=p[v[x]])   
                 cnt-=b[v[x]],b[v[x]]=0;
            else cnt+=1-b[v[x]],b[v[x]]=1;
        }
        if(x<n-1){
            if(lca(v[x],v[x+1])!=p[v[x+1]])
                 cnt-=b[v[x+1]],b[v[x+1]]=0;
            else cnt+=1-b[v[x+1]],b[v[x+1]]=1;
        }
        swap(x,y);
        if(x>0){
            if(lca(v[x-1],v[x])!=p[v[x]])   
