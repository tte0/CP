...
mt19937 mt(clock());
///////////////////////////////////////////////////////////////////
int n,m,k,t,q,a,b,x,y,w,ans,vis[N][N];
int adj[N][N][4];
vi v;

inline int dfs(int x,int y){
    if(x<=0 || y<=0 || n<x || m<y)return 0;
    if(vis[x][y]++)return 0;
}
...
inline void solve(void){
    ...
    for(int i=0;i<=n;i++){
        for(int j=0;j<m;j++){
            char c;
            input(c);
            if(c=='1')adj[i][j][1]=1,adj[i][j+1][3]=1;
        }
    }
    for(int i=0;i<n;i++){
        for(int j=0;j<=m;j++){
            char c;
            input(c);
            if(c=='1')adj[i][j][2]=1,adj[i+1][j][0]=1;
        }
    }

    for(int i=1;i<=n;i++){
        for(int j=1;j<=m;j++){
            if(adj[i][j][0]+adj[i][j][1]+adj[i][j][2]+adj[i][j][3]==4)continue;
            v.pb(dfs(i,j));
        }
    }
}
