...
mt19937 mt(clock());
///////////////////////////////////////////////////////////////////
int n,m,k,t,q,a,b,x,y,w,ans,root;
vi p,adj[N];

inline int dfs(vi& sz,int node=root){
    //debug("node|sz:",node,"|",sz);
    for(int i:adj[node])sz[node]+=dfs(sz,i);
    //debug("node|sz:",node,"|",sz);
    return sz[node];
}
inline int dfs2(vi& sz,int node=root){
    //debug("dfs2:",node);
    for(int i:adj[node]){
        int t=dfs2(sz,i);
        if(t!=-1)return t;
    ...
    //debug("dfs2:",node);
    return 3*sz[node]>=sz[root]?node:-1;
}

inline int findcentroid(){
    vi sz(n,1);
    dfs(sz);
    //debug("dfs ok");
    //debug("sz:",sz);
...
int solve(int n,vi p){
    ...
    while(adj[root].size()){
        ...
        int c=findcentroid();
        ASSERT(c==-1,"no centroid");
        if(ask(c+1))root=c;
        else{
            int parent=p[c];
            //debug("c,p[c]:",c,parent);
