...
inline int dsu(int x,vi& fa){
    return fa[x]<0?x:fa[x]=dsu(fa[x],fa);
}
 
inline bool merge(int x,int y,vi& fa){
    x=dsu(x,fa),y=dsu(y,fa);
    if(x!=y)fa[y]=x;
    return x!=y;
}
 
inline void binarysearch(int l,int r,vi& v,vi& fa){
    if(l>r)return;
    if(l==r){
    ...
    vi _fa=fa;
    for(int i=l;i<=mid;i++){
        merge(edges[i].ff,edges[i].ss,fa);
    }
 
    vi t,t2;
    while(v.size()){
        if(dsu(querys[v.back()].ff,fa)==dsu(querys[v.back()].ss,fa)){
            t.pb(v.back());
        }
        else{
            t2.pb(v.back());
    ...
    binarysearch(l,mid,t,_fa);
    binarysearch(mid+1,r,t2,fa);
}
...
inline void solve(void){
    ...
    vi v,fa(n,-1);
    for(int i=0;i<q;i++)v.pb(i);
 
    binarysearch(0,m,v,fa);
 
    for(int i=0;i<q;i++)print((querys[i].ff==querys[i].ss)?0:(ans[i]==m+1?-1:ans[i]));
}
