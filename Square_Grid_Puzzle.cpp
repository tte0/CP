...
vvi v;
vector<pair<str,vi>> ans;

inline bool sorted(){
    for(int i=0;i<n;i++){
        for(int j=0;j<n;j++){
            if(v[i][j]!=i*n+j)return false;
        }
    }
    return true;
}

inline void _sort(vi& t,int x){
    vvi arr(n);
    for(int i=0;i<n;i++){
        if(x==0)arr[t[i]/n].pb(t[i]);
        else    arr[t[i]%n].pb(t[i]);
    }
    //debug("arr:",arr);
    for(int i=0;i<n;i++){
        if(arr[i].empty())continue;
        sort(all(arr[i]));
        swap(arr[i][mt()%arr[i].size()],arr[i][mt()%arr[i].size()]);
    }
    vi ans(n,-1);
    for(int i=0;i<n;i++){
        if(arr[i].size()){
...
inline void solve(void){
    ...
    while(!sorted() && time--){
        for(int round=0;round<n;round++){
            vi t;
            for(int i=0;i<n;i++)t.pb(v[i][0]);
            _sort(t,0);
            ans.pb({"R",t});

            for(int j=0;j<n-1;j++)for(int i=0;i<n;i++)v[i][j]=v[i][j+1];
            for(int i=0;i<n;i++)v[i][n-1]=t[i];
        }
        //debug(v);
        for(int round=0;round<n;round++){
            vi t;
            for(int j=0;j<n;j++)t.pb(v[0][j]);
            _sort(t,1);
            ans.pb({"D",t});

            for(int i=0;i<n-1;i++)for(int j=0;j<n;j++)v[i][j]=v[i+1][j];
            for(int j=0;j<n;j++)v[n-1][j]=t[j];
        }
        //debug(v);
    }
