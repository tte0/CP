...
mt19937 mt(clock());
///////////////////////////////////////////////////////////////////
int n,m,k,t,a,b,x,y,w,ans,base[N],hashs[N];
str s,q;
...
inline void solve(void){
    ...
    int hash=0,cnts[N][26],cntq[26];
    for(int i=0;i<m;i++){
        hash=(hash+base[i]*(s[i]-'a'))%MOD;
        hashs[i+1]=hash;
        for(int j=0;j<26;j++)cnts[i][j+1]=cnts[i][j];
        cnts[i][s[i]-'a']++;
        cntq[q[i]-'a']++;
    }
    for(int i=0;i+m<n;i++){
        hash=(hash-base[i]*(s[i]-'a')+base[i]*(s[i+m]-'a'))%MOD;
        hashs[i+m+1]=hash;
        for(int j=0;j<26;j++)cnts[i][j+1]=cnts[i][j];
        cnts[i][s[i]-'a']++;
    }
