...
mt19937 mt(clock());
///////////////////////////////////////////////////////////////////
int n,m,k,t,q,a,b,x,y,w,ans,sum,cnt[N];
vi v,new_v,_const={0};

inline int query(vi v,vi new_v){
    sort(all(v));
    sort(all(new_v));
    int ans=0;
    for(int i=0;i<n;i++)ans+=t[i]*_const[i];
    return ans;
}
...
inline void solve(void){
    ...
    for(int i:v)cnt[i]++;
    for(int i=0;i<n;i++)_const.pb(i*(n-i-1));
    sort(all(_const));
    for(int i=1;i<=n;i++)_const[i]+=_const[i-1];
